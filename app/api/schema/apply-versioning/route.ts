...
/**
 * POST /api/schema/apply-versioning
 * Apply content versioning schema to database
 */
async function postHandler() {
  try {
    const supabase = createServiceClient()

    // Read the versioning schema from the script file
        ...
        WITH versions_to_delete AS (
          ...
          INNER JOIN (
            ...
            FROM content_entry_versions
                    ...
                    AND cev.version_number = ranked.version_number
          WHERE 
    ...
    const { data, error } = await supabase.rpc('execute_sql', {
      sql_query: versioningSchema
    })

    if (error) {
      console.error('Error executing versioning schema:', error)
      return NextResponse.json(
        { 
          success: false, 
          error: 'Failed to execute versioning schema',
    ...
    console.error('Unexpected error applying versioning schema:', error)
    return NextResponse.json(
      { 
        success: false, 
