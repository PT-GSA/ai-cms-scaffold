...
CREATE TABLE IF NOT EXISTS search_analytics (
  ...
  query text NOT NULL,
  query_type varchar(50) DEFAULT 'full_text', -- full_text, similarity, autocomplete
...
-- Search suggestions table (untuk autocomplete)
CREATE TABLE IF NOT EXISTS search_suggestions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  suggestion text NOT NULL UNIQUE,
  category varchar(100), -- title, tag, category, etc
  frequency integer DEFAULT 1,
  last_used timestamptz DEFAULT NOW(),
  created_at timestamptz DEFAULT NOW()
);

-- Index untuk search suggestions
...
CREATE INDEX IF NOT EXISTS idx_search_suggestions_frequency 
  ON search_suggestions (frequency DESC, last_used DESC);
...
BEGIN
  ...
  SELECT 
    ...
    -- Search ranking berdasarkan relevance
    ts_rank_cd(ce.search_vector, ts_query) as search_rank,
    
    -- Similarity score (jika diminta)
  ...
  WHERE 
    ...
    -- Only published content untuk public search
...
-- Function untuk fuzzy/similarity search
...
-- Function untuk generate search suggestions (autocomplete)
...
BEGIN
  -- Update atau insert suggestion berdasarkan usage
  ...
  WHERE 
    ss.suggestion % partial_query -- Trigram similarity operator
...
-- Populate initial search vectors untuk existing data
...
-- Function untuk search analytics
...
-- RLS policies untuk search analytics
ALTER TABLE search_analytics ENABLE ROW LEVEL SECURITY;
ALTER TABLE search_suggestions ENABLE ROW LEVEL SECURITY;

-- Policy untuk search analytics (admin only)
...
-- Policy untuk search suggestions (public read)
...
-- Create maintenance function untuk cleanup old analytics
